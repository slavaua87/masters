...
double normal_logcopula_c(vec cop_value, mat sqrt_rho) {
  ...
  if (any(test)) {
    marginals.elem(find(test)).fill(numeric_limits<double>::min());
  }
  ...
  test = rho_diag == 0.0;
  if (any(test)) {
    rho_diag.elem(find(test)).fill(numeric_limits<double>::min());
  }
...
//  # 4:lambda, 5:gamma, 6:chi, 7:phi, 8:rho_db, 9:rho_dt, 10:rho_bt, 11:omega
  ...
  if (any(test_bounds)) {
    marginal_dens.elem(find(test_bounds)).fill(numeric_limits<double>::min());
  }
  ...
  if (any(test_bounds)) {
    cop_value.elem(find(test_bounds)).fill(numeric_limits<double>::min());
  }
...
//[[Rcpp::export]]
double calc_density_integrand_c(vec x, double rt, double choice,
...
//  # 4:lambda, 5:gamma, 6:chi, 8:phi, 9:rho_db, 10:rho_dt, 11:rho_bt, 12:omega
  ...
  double integrand = exp(sum(log(behav_dens) + param_logdens + log(jacobian)));
  if (isnan(integrand) || integrand == datum::inf) 
    integrand = numeric_limits<double>::min();
