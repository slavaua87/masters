...
### Generates trial parameters from the joint density of drift, bias, nondecision
library(copula)
library(tmvtnorm)
...
simul_copula <- function(smpl_size, params, model) {
  ...
  cop_pdf <- ellipCopula(family = model, 
                         param = as.numeric(params["rho_db", "rho_dt", "rho_bt"]), 
                         dim = 3, dispstr = 'un', 
...
calc_param <- function(cop_smpl, params) {
  ...
  normal_quantiles <- sapply(X = cop_smpl[, 3],
                             FUN = qtmvnorm.marginal,
  ...
  param_smpl[, "delta"] <- qnorm(cop_smpl[, 1], as.numeric(params["nu"]),
                           as.numeric(params["eta"]))
  param_smpl[, "beta"] <- qbeta(cop_smpl[, 2], shape1, shape2)
  param_smpl[, "t_nd"] <- unlist(normal_quantiles["root", ])
  return(param_smpl)
}
...
smpl_param <- function(params, smpl_size, model) {
  ...
  param_smpl <- as.data.frame(calc_param(cop_smpl = cop_smpl, params = params))
  return(param_smpl)
}
