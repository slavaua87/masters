...
times <- read.table("results/sample_path/times.txt", header = TRUE) %>%        
  as_data_frame %>%
  arrange(model, condition, desc(response))

time_diff <- function(times) {
  diffs <- dplyr::select(times, len) -
    filter(times, model == "independent") %>%
    dplyr::select(len) %>%
    unlist %>%
    rep(times = 3)
  return(unlist(diffs))
...
filter(times, condition %in% 37:42) %>%
  mutate(diffs = time_diff(.)) %>%
  filter(model == "normal") %>%
  as.data.frame
